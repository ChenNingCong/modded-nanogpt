...
if __name__ == "__main__":
    ...
    parser.add_argument("--project_id", type=str, required=True, help="Project id for wandb")
    ...
    # compile happens after DDP
    if args.compile:
        if hasattr(config, "coordinate_descent_tuning"):
            config.coordinate_descent_tuning = True # suggested by @Chillee
        print0("compiling the model...")
        compiler_db_path = "compiled_db"
        use_compile_cache = os.path.exists(compiler_db_path) 
        if use_compile_cache:
            artifact_bytes, cache_info = torch.load(compiler_db_path, weights_only=False)
            torch.compiler.load_cache_artifacts(artifact_bytes)
        model = torch.compile(model, dynamic=True, mode="reduce-overhead", fullgraph=False)
    ...
    if args.write_tensors and (not args.inference_only):
        ...
        if master_process:
            ...
            write_model(model, os.path.join(args.output_dir, f"gpt2_{model_size_str}.bin"), dtype="float32")
    ...
    if master_process:
        import wandb
        wandb.init(
            project=args.project_id, # Replace with your project name
            name=args.run_id,
            config=vars(args) if args is not None else None,
            save_code=True
            # other parameters like entity, name, group can be added
        )
        wandb.watch(model, log="all", log_freq=args.val_loss_every) 
    if device == "cuda":
    ...
    for step in range(args.num_iterations + 1):
        ...
        if (args.val_loss_every > 0 \
            ...
            wandb.log({"val/loss": val_loss}, step=step, commit=False) 
        ...
        wandb.log({"train/loss": lossf, "train/norm" : norm, "train/lr" : lr, "train/tokens_per_second" : tokens_per_second}, step=step, commit=True) 
        # log to logile
        if master_process and logfile is not None:
            with open(logfile, "a") as f:
                f.write("s:%d trl:%f\n" % (step, lossf))
        ...
        # Compile the code after first step
        if step == 0 and args.compile and  not use_compile_cache:
            artifacts = torch.compiler.save_cache_artifacts()
            assert artifacts is not None
            torch.save(artifacts, compiler_db_path)
    # print the average of the last 20 timings, to get something smooth-ish
    timings = timings[-20:]
    print0(f"final {len(timings)} iters avg: {np.mean(timings)*1000:.3f}ms")
    print0(f"peak memory consumption: {torch.cuda.max_memory_allocated() // 1024 // 1024} MiB")
    write_model(model, os.path.join(args.output_dir, "checkpoint.bin"), dtype="bfloat16")
    # -------------------------------------------------------------------------
    # clean up nice
    if ddp:
